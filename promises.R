...
# Synchronous completion blocks Shiny reactivity:
...
# Synchronous completion allows Shiny reactivity if `run_now(all = FALSE)`:
...
mirai(2) |> then(print) |> then(print)
promise_resolve(3) |> then(print)
while (!later::loop_empty()) {later::run_now(Inf, all = FALSE); print("Shiny Reactivity")}
